...
class NovaType:
    ...
    def __eq__(self, other):
        if not isinstance(other, NovaType):
            return False
        return self.kind == other.kind and self.params == other.params and self.name == other.name
...
class MIRMapBuild(MIRInstruction):
    """MIR Map 构建"""
    entries: List[Tuple[str, str]] = field(default_factory=list)   # [(key_ssa, val_ssa), ...]
...
class MIRPhi(MIRInstruction):
    """MIR SSA phi 节点"""
    sources: List[Tuple[str, str]] = field(default_factory=list)   # [(block_label, ssa_name), ...]
...
class MIRSwitch(MIRTerminator):
    """MIR switch 跳转"""
    value: str = ""            # SSA 名
    cases: List[Tuple[Any, str]] = field(default_factory=list)   # [(value, target_block), ...]
    default_target: str = ""
...
class MIRMatchJump(MIRTerminator):
    """MIR match 跳转"""
    value: str = ""
    variant_tests: List[Tuple[str, List[str], str]] = field(default_factory=list)   # [(variant_name, fields, target_block), ...]
    default_target: str = ""
