...
| 最大短板 | 代码生成正确性隐忧 | 多个后端存在静默生成错误代码的风险 |

**各后端完成度变化**：

| 后端 | 第 48 轮 | 第 51 轮 | 变化 | 关键事件 |
|------|----------|----------|------|----------|
| C 后端 | ~88% | **~85%** | -3% | 审计发现闭包浮点返回丢失（已知 bug 未修复） |
| 原生后端 | ~90% | **~88%** | -2% | 审计发现浮点全局存储错误 + 外部调用偏移为0 |
| Wasm 后端 | ~78% | **~78%** | 持平 | 内存泄漏已修复，栈平衡仍待验证 |
| Cranelift | ~30% | **~30%** | 持平 | **审计发现致命缺陷：TODO 降级静默生成错误代码** |

**后端新发现的关键缺陷（按严重性排序）**：

1. **P0-新2：Cranelift 后端 TODO 降级（P98, easy）**
   - `cranelift_backend.py _compile_instr` 对未处理指令发射 `;; TODO` 注释而非抛 `NotImplementedError`
   - 编译"成功"但生成的 Cranelift IR 缺少关键指令，执行结果不可预测
   - **这是安全优先问题，必须立即修复**

2. **P1-新2：Native 后端浮点全局变量存储错误（P85, medium）**
   - `_emit_store_global` 对浮点值先发射 `mov [rip], RAX`，然后注释说"撤销"但实际 `pass`
   - 没有任何撤销或重发射浮点存储，浮点全局变量赋值完全错误

3. **P1-新3：C 后端闭包间接调用浮点返回丢失（P80, medium）**
   - `_compile_call_indirect` 将 `nova_closure_call` 返回值统一当作 `int64_t`/`void*`
   - 未对 `double` 返回做 `memcpy` 解包，与 trampoline 的 `malloc+memcpy` 逻辑不匹配

4. **P2-E：Native 外部运行时调用偏移为0（P70, hard）**
   - `link_calls` 中对运行时函数（`nova_init`、`nova_list_new` 等）保持 0 偏移
   - 调用这些函数会直接跳转到错误地址导致段错误
...
| 问题 | 根因 | 影响 | 修复策略 |
|------|------|------|----------|
| Cranelift TODO 降级 | 开发初期为方便调试将异常改为注释，未改回 | 编译成功但执行结果不可预测 | 1 行修改：TODO 注释 → 抛 NotImplementedError |
| Native 浮点全局存储 | 复制粘贴整数存储代码后未实现浮点分支 | 浮点全局变量赋值完全无效 | 区分整数/浮点路径，浮点用 movsd |
| C 闭包浮点返回 | trampoline 和解包逻辑不匹配 | 闭包返回 double 结果错误 | 对齐 trampoline 的 malloc+memcpy 解包逻辑 |
| Native 外部调用链接 | 运行时函数未纳入链接符号表 | 调用运行时函数段错误 | 实现 PLT/GOT 或硬编码偏移 |
| Parser 块边界吞错 | 错误恢复逻辑过于宽容 | 错误语法被静默接受 | 缺失分隔符时抛 ParseError |

**共性根因**：
1. **" TODO 注释代替异常"的反模式** — Cranelift 和早期 Native 都有此问题，第 40 轮已修复三后端静默跳过，但 Cranelift 遗漏
2. **浮点路径覆盖不全** — Native 和 C 后端都有浮点特殊处理遗漏，反映浮点测试覆盖不足
3. **链接/重定位复杂性被低估** — Native 后端从 obj+gcc 链接模式过渡到独立 ELF 时，运行时函数链接未完全解决
...
**第 52 轮（普通轮）**：
- **前端**：无（前端进入纯维护模式，Parser 块边界问题推迟到第 53 轮）
- **后端**：`backend_cranelift_todo_fatal`（P98, easy）+ `backend_native_float_global_store`（P85, medium）
- **理由**：Cranelift 安全缺陷是 P0 级，1 行修改即可防止生成错误代码；Native 浮点全局存储是已知功能缺失，修复后提升 Native 后端正确性

**第 53 轮（普通轮）**：
- **前端**：`frontend_parser_block_boundary`（P55, easy）
- **后端**：`backend_c_closure_double_return`（P80, medium）
- **理由**：前端剩余的唯一审计问题；C 后端闭包浮点返回补齐 C 后端闭包功能完整性

**第 54 轮（评审轮）**：
- 全面评审第 52-54 轮成果
- 完成 `backend_wasm_stack_balance`（P45）和 `backend_lir_phi_lowering_verify`（P42）
- 评估 `backend_native_runtime_link`（P70, hard）的进展
...
| 任务 ID | 名称 | 优先级 | 来源 | 理由 |
|---------|------|--------|------|------|
| backend_cranelift_todo_fatal | 修复 Cranelift TODO 降级为致命异常 | **98** | code_audit_51 | P0 级安全缺陷，1 行修改，防止静默生成错误代码 |
| backend_native_float_global_store | 修复 Native 浮点全局变量存储 | **85** | code_audit_51 | 功能缺失，浮点全局变量赋值完全错误 |
| backend_c_closure_double_return | 修复 C 后端闭包浮点返回丢失 | **80** | code_audit_51 | 功能缺失，闭包返回 double 结果错误 |
| frontend_parser_block_boundary | 修复 Parser 块边界静默吞错 | **55** | code_audit_51 | 边界语法处理缺陷，错误语法被静默接受 |
| backend_native_runtime_link | 修复 Native 外部运行时调用偏移为0 | **70** | code_audit_51 | 链接缺陷，调用运行时函数段错误 |

**保留 2 个原有任务**：
- `backend_wasm_stack_balance`（P45）
- `backend_lir_phi_lowering_verify`（P42）
...
| 优先级 | 任务 | 难度 | 计划轮次 |
|--------|------|------|----------|
| 98 | 修复 Cranelift TODO 降级为致命异常 | easy | 第 52 轮 |
| 85 | 修复 Native 浮点全局变量存储 | medium | 第 52 轮 |
| 80 | 修复 C 后端闭包浮点返回丢失 | medium | 第 53 轮 |
| 70 | 修复 Native 外部运行时调用偏移为0 | hard | 第 54 轮 |
| 55 | 修复 Parser 块边界静默吞错 | easy | 第 53 轮 |
| 45 | 实现 Wasm 后端栈平衡验证器 | medium | 第 54 轮评审 |
| 42 | 验证 Phi 节点 LIR 降级正确性 | medium | 第 54 轮评审 |
...
| 轮次 | 第 48 轮（评审轮） |
| 评审范围 | 第 46-48 轮（3 轮） |
| 测试基线 | 698 passed, **1 failed**（test_e2e_loop 回归） |
| 前端完成率 | 32/32 = **100%** |
| 后端完成率 | 35/48 = **72.9%** |
| 总完成率 | 67/80 = **83.8%** |
| 前端质量评分 | **8/10**（+0.5 vs 第 45 轮 7.5/10） |
| 后端质量评分 | **7/10**（+1.0 vs 第 45 轮 6/10） |
| 失败任务 | 0 个 |
| 新增任务 | 2 个（backend_native_regalloc_loop_regression, frontend_try_expr_span_fix） |

---

### 三轮回顾总结

#### 第 46 轮（普通轮）— Native 端到端执行突破

- **前端**：修复 type_checker.py Set 导入 + try 表达式 TypeVar 放行（FRONTEND-031，easy）
- **后端**：Native 后端端到端执行突破（backend_native_elf_e2e_exec，hard，P80）— 修复 4 个核心 bug，首次实现 Native 后端可执行，新增 6 个 E2E 测试
- **成果**：P1-B3 清零，Native 完成度从 ~85% 提升至 ~90%

#### 第 47 轮（普通轮）— 寄存器分配器优化

- **前端**：修复 parser.py 块错误恢复计数器未重置（FRONTEND-032，easy）
- **后端**：寄存器分配器添加调用点活跃区间切口（backend_native_regalloc_call_site，hard，P55）— 从全保存 8 个 caller-saved 改为精确保存，P1-B2 清零
- **成果**：新增 3 个测试。**但第 48 轮评审发现引入循环回归**
...
| 维度 | 评估 |
|------|------|
| 质量趋势 | **持续优秀（8/10）**。类型系统合一算法含 occur check，let-polymorphism 正确实现，match 完备性检查覆盖 4 种类型。调度表模式控制圈复杂度。 |
| 进度评估 | 32/32 = **100%**，进入纯维护模式。三轮完成 2 个维护任务（Set 导入+TypeVar 放行、块错误计数器重置）。 |
| 价值评估 | 前端维护任务价值递减，但保持代码整洁。 |
| 薄弱点 | `_check_try_expr` 属性名错误（expr.expr.line 应为 expr.expr.span.line），位置信息丢失但不崩溃。Phi 类型推断取第一个源类型未做合一（理论风险）。 |

#### 后端评估

| 维度 | 评估 |
|------|------|
| 质量趋势 | **稳健提升（7/10）**。Native 后端从原型到可执行（第 46 轮突破），寄存器分配器从保守到精确（第 47 轮）。但引入回归说明测试覆盖不足。 |
| 进度评估 | 35/48 = **72.9%**。Native ~90%，C 后端 ~85%，Wasm ~75%。 |
| 价值评估 | Native 端到端执行（P80）是最高价值任务，已实现。 |
| 薄弱点 | **最大短板：第 47 轮寄存器分配器改动引入 test_e2e_loop 回归**（P0 级）。Wasm 后端缺少端到端验证。闭包内存泄漏（P2-D）。 |

#### 综合评估

| 维度 | 结论 |
|------|------|
| 前后端平衡 | 不平衡但合理。前端 100% 完成，后端 72.9%。后端复杂度远高于前端（8289 行 vs 4253 行）。 |
| 方向 | 正确。Native 后端从原型到可执行是最大成就。 |
| 效率 | 三轮完成 4 个任务 + 1 次评审，平均每轮 1.3 个任务。 |
| 最大问题 | **回归未被发现**。第 47 轮的 3 个新测试通过，但 test_e2e_loop 回归未被捕获，因为当时测试可能未在评审环境运行。 |

---

### 问题总结与根因分析

#### P0 级回归：test_e2e_loop 失败

**现象**：循环 `sum(0..9) = 45` 返回 10（只循环了 1 次）。

**根因**：第 47 轮 `_allocate_registers` 步骤 3 计算 `caller_saved_to_preserve` 时，可能错误地将循环变量（i/sum）所在寄存器判定为"调用点后不活跃"，导致循环变量值在 call 指令处被覆盖。活跃区间计算可能未正确处理循环回边（loop back-edge），将循环变量的 last_use 截断在 call 指令之前。

**验证方法**：用第 46 轮代码（ac0e13b）运行 test_e2e_loop 通过（返回 45），用第 47 轮代码（08f8368）运行失败（返回 10）。

**影响**：Native 后端所有含循环+函数调用的程序都会产生错误结果。

**修复方向**：检查活跃区间计算是否正确处理回边；确保循环变量在 call 指令后的活跃区间不被错误截断；如果循环内无 call 指令，则 caller_saved_to_preserve 应为空。

#### 审计新发现

| 发现 | 位置 | 影响 | 优先级 |
|------|------|------|--------|
| Native 闭包创建仍全保存 caller-saved | native_backend.py _emit_closure_create | 第 47 轮优化遗漏点 | 中 |
| 前端 _check_try_expr 属性名错误 | type_checker.py:1059-1060 | 位置信息丢失 | 低 |
| Wasm _compile_index_access 类型不安全 | wasm_backend.py:810-820 | 大索引可能溢出 | 低 |

---

### 下阶段方向与理由

**第 49-51 轮聚焦方向（按优先级排序）：**

1. **第 49 轮：修复寄存器分配器循环回归（P0，P95）**
   - 理由：P0 级回归，阻塞 Native 后端正确性，必须立即修复
   - 涉及：backend/native_backend.py _allocate_registers 活跃区间计算

2. **第 49 轮（并行）：修复前端 _check_try_expr 属性名（P40）**
   - 理由：小修复，30 分钟可完成，清理技术债

3. **第 50 轮：修复 C/Wasm 后端闭包临时内存泄漏（P50）**
   - 理由：正确性缺陷（内存泄漏），影响 C 和 Wasm 两个后端

4. **第 50-51 轮：实现 Wasm 后端栈平衡验证器（P45）**
   - 理由：Wasm 后端可用性的关键瓶颈

5. **第 51 轮：验证 Phi 节点 LIR 降级正确性（P42）**
   - 理由：IR 管线质量保障
...
| 变更 | 任务 | 原因 |
|------|------|------|
| **新增** | backend_native_regalloc_loop_regression (P95) | 第 47 轮引入的 P0 回归 |
| **新增** | frontend_try_expr_span_fix (P40) | 第 48 轮审计发现属性名错误 |
| 保持 | backend_closure_memory_leak_fix (P50) | 第 45 轮规划，优先级不变 |
| 保持 | backend_wasm_stack_balance (P45) | 第 45 轮规划，优先级不变 |
| 保持 | backend_lir_phi_lowering_verify (P42) | 第 45 轮规划，优先级不变 |

**任务池当前状态**：5 个待做任务（后端 4 + 前端 1），1 个 P0 级。
...
|------|------|
| 评审区间 | 第 43-45 轮（3 轮） |
| 测试基线 | 621 passed, 20 subtests |
| 前端完成率 | 30/30 = **100%** |
| 后端完成率 | 33/46 = **71.7%** |
| 总完成率 | 63/76 = **82.9%** |
| P0 问题 | 全部清零 |
| P1 问题 | 2 个待改进 + 1 个部分清零 |
| 新增任务 | 2 个（Native 端到端执行、闭包内存泄漏） |
| 降级任务 | 1 个（统一 C 后端） |
| 失败任务 | 0 个（三轮零失败） |
...
**第 43 轮**: 前端修复 ForExpr step 字段不一致（easy, P70），后端建立三后端统一闭包执行测试矩阵（medium, P72）。P1-B3 部分清零——C 后端 E2E 完全通过，Native 编译基线建立。测试 572 passed（+6 新测试）。

**第 44 轮**: 前端修复 parser match 字符字面量模式缺失（easy, P55），后端修复 Native 可重定位 ELF 链接格式（hard, P60）。5 个 ELF 格式 bug 修复 + SIB 字节支持。测试 572 passed。

**第 45 轮**: 评审轮。深度代码审计发现 3 个新问题，新增 2 个高价值任务。
...
| 三轮任务成功率 | 4/4 = **100%** |
| 三轮零失败 | 是 |
| 测试增长 | 572 → 621（+49，含外部贡献） |
| P0 清零数 | 0（已全部清零） |
| P1 推进 | P1-B3 部分清零（.o 可链接，端到端执行待修复） |
...
| 维度 | 评估 |
|------|------|
| type_checker.py | 2000+行，HM unification + union-find + occur check，逻辑完整 |
| parser.py | 递归下降解析器，10处 ParseError，错误恢复完善 |
| lexer.py | 转义处理完整，无技术债 |
| 测试覆盖 | 49 个类型检查测试，覆盖良好但缺少多态递归/嵌套 match 边界测试 |

**审计发现（非阻塞）**：
- Phi 类型推断取第一个源 SSA 类型未做合一验证（mir_lowering.py:860）
- 缺少 let-polymorphism value restriction（理论风险，Nova 当前无此场景）

#### 后端质量评估：6/10

| 后端 | 完成度 | 审计发现 |
|------|--------|---------|
| C 后端 | ~95% | 闭包临时数组未 free（nova_alloc 后无 free） |
| 原生后端 | ~85% | 独立 ELF 跳过外部函数重定位（第1668行 continue）；寄存器分配器活跃区间基于指令索引；无端到端执行测试 |
| Wasm 后端 | ~80% | Unit 返回值缺 drop；return 策略不一致（return vs br $exit）；闭包临时内存泄漏 |
| Cranelift | <30% | 仅框架 |

**关键发现**：
1. **P2-F（最高风险）**：native_backend.py `_generate_elf` 第 1668 行 `continue` 跳过外部函数重定位，导致独立 ELF 无法链接运行时函数
2. **P2-D**：C/Wasm 后端闭包创建和调用时 nova_alloc 分配的临时数组均未释放
3. **P2-E**：Wasm 后端 Unit 返回值缺少 drop 处理，可能导致 wat2wasm 验证失败
...
前端代码质量稳定在 7.5-9.0/10。30/30 任务全部完成，零技术债。进入纯维护模式。

**进度评估：完全完成**

前端任务池已空，后续仅响应审计发现的新正确性问题。

**薄弱点：无阻塞**

审计发现的两个问题（Phi 类型推断、value restriction）均为理论风险，实际运行无问题，优先级低。

#### 后端线

**质量趋势：稳健改善**

三轮内完成了 ELF 链接格式修复、统一闭包测试矩阵建立。P0 全部清零，P1 持续推进。但深度审计发现 3 个新的 P2 级问题。

**进度评估：71.7%，进入收尾攻坚期**

后端剩余 5 个待做任务，核心瓶颈从"功能实现"转向"正确性验证"和"端到端执行"。

**价值评估：端到端执行是最高价值**

| 任务 | 优先级 | 价值 |
|------|--------|------|
| Native 端到端执行突破 | **P80** | 彻底清零 P1-B3，验证原生后端编译正确性 |
| 寄存器分配优化 | P55 | 消除 P1-B2，提升性能 |
| 闭包内存泄漏修复 | P50 | 消除 P2-D，内存安全 |

**薄弱点：端到端验证不足**

Native 后端虽有完整的指令级单元测试，但从未验证编译后的程序能正确运行。这是当前最大短板。

#### 综合评估

**前后端平衡度：预期内不平衡**

前端 100% 完成进入维护模式，后端 71.7% 仍在活跃开发。下阶段全部投入后端。

**方向评估：方向正确，需聚焦端到端**

第 43-44 轮聚焦 ELF 链接格式修复和测试矩阵建立，方向正确。但审计发现独立 ELF 模式仍有外部函数重定位问题，需要下阶段突破。
...
| 问题 | 根因 | 建议 |
|------|------|------|
| Native 独立 ELF 无法运行 | _generate_elf 跳过外部函数重定位（第1668行 continue） | **第 46 轮**：为外部函数生成 PLT/GOT 重定位或改用 gcc 链接模式 |
| 闭包临时内存泄漏 | C/Wasm 后端 nova_alloc 后无 free 调用 | **第 47-48 轮**：添加 free() 调用或引入 arena allocator |
| Wasm 栈平衡隐患 | Unit 返回值无 drop + return 策略不一致 | **第 48 轮**：添加 drop 处理，统一为 br $exit |
| 寄存器分配低效 | 线性扫描不感知调用点 | **第 47 轮**：添加调用点活跃区间切口 |
...
| 任务 | 优先级 | 来源 | 理由 |
|------|--------|------|------|
| backend_native_elf_e2e_exec | **P80** | 第 45 轮审计 | P2-F：修复外部函数重定位 + 添加端到端执行测试，彻底清零 P1-B3 |
| backend_closure_memory_leak_fix | **P50** | 第 45 轮审计 | P2-D：修复 C/Wasm 后端闭包临时内存泄漏 |

**降级：1 个**

| 任务 | 原优先级 | 新状态 | 理由 |
|------|---------|--------|------|
| 统一 C 后端（旧路径迁移） | P40 | 降为维护模式 | 价值低于端到端执行和内存安全，旧路径不阻塞功能 |
...
**聚焦主题：Native 后端端到端执行突破 + 内存安全加固**

下 3 轮全部投入后端，前端仅响应新发现的正确性问题。

| 轮次 | 推荐任务 | 预期价值 |
|------|---------|----------|
| **第 46 轮** | backend_native_elf_e2e_exec (P80, hard) | 修复外部函数重定位，添加端到端执行测试，彻底清零 P1-B3 |
| **第 47 轮** | backend_native_regalloc_call_site (P55, hard) + backend_closure_memory_leak_fix (P50, medium) | 寄存器分配优化 + 内存泄漏修复 |
| **第 48 轮** | backend_wasm_stack_balance (P45, medium) + backend_lir_phi_lowering_verify (P42, medium) | Wasm 栈平衡验证 + Phi 降级验证 |

**策略调整**：
- 前端进入纯维护模式，不再主动开发新任务
- 后端聚焦端到端执行（P80）> 性能优化（P55）> 内存安全（P50）> 预防性验证（P42/P45）
- 第 46 轮只做 1 个 hard 任务（端到端执行），深度优先
