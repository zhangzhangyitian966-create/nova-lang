...
**状态：维护模式（22/25 完成，2 个待做正确性修复）**

前端核心功能已完成，进入维护模式。当前聚焦修复审计发现的正确性 bug。

### 待做任务（2 个）

| 状态 | 任务 | 难度 | 优先级 | 预计 | 轮次计划 |
|------|------|------|--------|------|----------|
| 待做 | 修复管道操作符类型检查语义错误 | medium | **82** | 2-3小时 | **第 34 轮** |
| 待做 | 修复递归函数类型推断不支持相互递归 | medium | **78** | 3-4小时 | **第 35 轮** |
...
|------|------|------|--------|------|------|----------|
| 待做 | 原生后端链接器战略决策与实现 | hard | **95** | 1-2天 | - | **第 35 轮** |
| 待做 | 实现 Wasm 后端闭包 fn_ptr 回填 | hard | **90** | 4-6小时 | wasm_closure_impl | **第 34 轮** |
| 待做 | 修复 C 后端 trampoline double 返回值 UB | easy | **55** | 1-2小时 | - | **第 34-35 轮** |

### 其他待做任务

| 状态 | 任务 | 难度 | 优先级 | 预计 | 依赖 |
|------|------|------|--------|------|------|
| 待做 | 建立三后端统一闭包执行测试矩阵 | medium | 72 | 4-6小时 | wasm_fn_ptr |
...
| P0-2 | wasm_backend _compile_closure_create fn_ptr 传 NULL | Wasm 闭包完全不可用 | 待修复（backend_wasm_fn_ptr, P90） |
...
| 编号 | 问题 | 影响 | 状态 |
|------|------|------|------|
| P1-F1 | 管道操作符类型检查 unify 副作用污染 + 静默返回 | 管道表达式类型推断不正确 | 待修复（frontend_pipe_typecheck_fix, P82） |
| P1-F2 | 递归函数类型推断不支持相互递归 | 相互递归函数编译失败 | 待修复（frontend_recursive_fn_typecheck, P78） |
| P1-B2 | 寄存器分配器不感知调用点 | 靠手动 push 兜底，低效 | 待改进（backend_native_regalloc_call_site, P52） |
| P1-B3 | 原生后端无端到端执行测试 | 正确性无行为验证 | 待补充（依赖 P0-B1） |
| P1-3 | type_checker.py 1905 行大文件病 | 维护成本高 | P2 技术债，暂不拆分 |
...
| P2-F5 | type_checker.py 1905 行单文件病，CC=39 | 维护成本高 | 暂不拆分（技术债） |
...
- 前端线：22/25 完成（含 1 废弃），2 个正确性 bug 待修（P1-F1/F2）
- 后端线：下 3 轮聚焦 P0 清零（Wasm fn_ptr + 链接器决策）+ C 后端 double UB
- 闭包进度：MIR 降级已完成（含鲁棒性修复），LIR 降级 SSA callee 已修复，三后端中 C 后端最完整（trampoline+fn_ptr），Native 已修复（trampoline 方案），Wasm fn_ptr 待回填
- P0 修复进度：4 个 P0 中 P0-1/P0-3 已清零，剩余 P0-2（wasm fn_ptr, P90）和 P0-B1（native 无链接器, P95, 新发现）
- P1 修复进度：P1-1/P1-A 已清零，P1-4 部分清零，新增 P1-F1/F1（前端正确性）和 P1-B2/B3（后端）
- 第 33 轮评审新增 5 个任务：frontend_pipe_typecheck_fix(P82)、frontend_recursive_fn_typecheck(P78)、backend_native_linker_strategy(P95)、backend_unified_closure_e2e_test(P72)、backend_native_regalloc_call_site(P52)
- 每轮开发：1 个前端任务 + 1 个后端任务（前端维护模式时可为轻量增量）
- 每 3 轮一次评审，调整优先级和任务池

## 评审记录

- **第 33 轮评审**（2026-07-26）：第十一次三轮回顾（第 31-33 轮）。深度代码审计 13 个核心文件 12940 行。新发现 P0-B1（native 无链接器，战略级盲点）。确认 P0-2（wasm fn_ptr NULL）、P2-A（C double UB）仍存在。前端质量 88/100（+2），维护模式，新增 2 个正确性 bug（P1-F1 管道操作符、P1-F2 递归推断）。后端 C 75%/Native 68%/Wasm 58%。前后端失衡加剧（95.7% vs 75.8%）。新增 5 个任务，下 3 轮聚焦 P0 清零 + 前端正确性 + 执行基线。
- **第 30 轮评审**（2026-07-26）：第十次三轮回顾（第 28-30 轮）。发现 2 个 P0、2 个新 P1、2 个新 P2。前端质量 86/100（+1）。后端 C 75%/Native 62%/Wasm 58%。新增 3 个任务。
