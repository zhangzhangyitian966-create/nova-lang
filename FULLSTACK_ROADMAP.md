...
|------|------|------|--------|------|------|----------|
| **待做** | **修复原生后端 _emit_runtime_call P0 bug** | **easy** | **99** | **2-3小时** | - | **第 22 轮** |
...
| 编号 | 问题 | 影响 | 修复方案 |
|------|------|------|----------|
| P0-1 | _emit_runtime_call 引用未定义的 CALLER_GPRS/INT_ARG_REGS/FLOAT_ARG_REGS | 原生后端所有运行时调用 NameError | 提升为模块级常量 |
| P0-2 | _emit_build_tuple 浮点字段负栈偏移 | 可能覆盖栈上关键数据 | 改用正偏移临时区 |
| P0-3 | Wasm 间接调用引用未声明的 $nova_closure_call | Wasm 闭包调用必然失败 | 整体重写为闭包实现 |

## 说明

- 前端线：**已全部完成（14/14），进入维护模式**
- 后端线：下 3 轮聚焦 P0 bug 修复 + 闭包全链路
