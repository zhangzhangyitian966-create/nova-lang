...
### 高优先级任务（下 3 轮聚焦，第 42 轮评审）

| 状态 | 任务 | 难度 | 优先级 | 预计 | 依赖 | 轮次计划 |
|------|------|------|--------|------|------|----------|
| 待做 | 修复三后端未实现指令静默跳过问题 | easy | **90** | 1-2小时 | - | **第 40 轮** |
| 待做 | Native 后端实现全局变量加载/存储指令 | medium | **82** | 3-4小时 | - | **第 41 轮** |
| 待做 | 建立三后端统一闭包执行测试矩阵 | medium | **72** | 4-6小时 | - | **第 42 轮** |

### 其他待做任务

| 状态 | 任务 | 难度 | 优先级 | 预计 | 依赖 |
|------|------|------|--------|------|------|
| 待做 | 修复 C 后端未实现指令静默生成注释问题 | easy | 60 | 1小时 | - |
...
| 排名 | 后端 | 完成度 | 关键缺失 |
|------|------|--------|----------|
| 1 | **C 后端** | **~95%** | 静默生成 TODO 注释（应改报错） |
| 2 | **Wasm 后端** | **~90%** | 静默生成 TODO 注释；栈平衡未验证 |
| 3 | **原生后端** | **~85%** | 缺失全局变量指令 + 静默跳过未实现指令 |
...
| 编号 | 问题 | 影响 | 状态 |
|------|------|------|------|
| **P0-新1** | **三后端未实现指令静默跳过/注释** | **编译可能成功但生成错误代码** | **待修复**（backend_unified_silent_skip_fix, P90） |
| **P1-新1** | **Native 后端缺失 LIRLoadGlobal/LIRStoreGlobal** | **全局变量访问被静默丢弃** | **待修复**（backend_native_global_var_support, P82） |
| P1-3 | C 后端未实现指令静默生成注释 | 可能生成无法编译的 C 代码 | 待修复（可合并到 P0-新1） |
