...
|------|------|------|--------|------|------|----------|
| 待做 | 建立三后端统一闭包执行测试矩阵 | medium | **72** | 4-6小时 | - | **第 43 轮** |
...
| P1-B2 | 寄存器分配器不感知调用点 | 靠手动 push 兜底，低效 | 待改进（第 44 轮计划） |
| P1-B3 | 原生后端无端到端执行测试 | 正确性无行为验证 | 待补充（第 43 轮计划） |
...
| P1-B3 | 原生后端无端到端执行测试 | 正确性无行为验证 | 待补充（backend_unified_closure_e2e_test, P72, 第 43 轮） |
