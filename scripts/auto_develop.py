...
class AutoDeveloper:
    ...
    def run_task(self, task):
        ...
        backup_tag = f"dev-backup-{task.task_id}-{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def generate_report(cycle_num, results, test_before, test_after):
    ...
    for result in results:
        icon = {'completed': '✅', 'failed': '❌', 'rolled_back': '↩️'}.get(result['status'], '❓')
        status = {'completed': '已完成', 'failed': '实现失败', 'rolled_back': '验证失败已回滚'}.get(result['status'], '未知')
    ...
    lines.append("")
    total = len(ALL_TASKS)
    done = len([t for t in ALL_TASKS if t.is_completed() or t.task_id in [r['task_id'] for r in results if r['status'] == 'completed']])
...
def generate_roadmap():
    ...
    for cat, tasks in sorted(categories.items()):
        ...
        for task in sorted(tasks, key=lambda t: -t.priority):
            ...
            lines.append(f"| {status} | {task.name} | {task.difficulty} | {task.priority} | {task.estimated_effort} |")
        lines.append("")
...
def git_commit_and_push(cycle_num, completed_count):
    ...
    stdout, stderr, rc = run_cmd(
        ["git", "commit", "-m", f"auto: 第 {cycle_num} 轮自动开发 - {completed_count} 个功能 (v1.0)"]
    )
    if rc != 0:
...
def main():
    ...
    report = generate_report(cycle_num, developer.results, test_str_before, test_str_after)
